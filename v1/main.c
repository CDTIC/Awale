...
int main(int argc, char *argv[])
{
    ...
    if(informations.id==1)
    {
        ...
        Mix_Chunk *moveEffect = Mix_LoadWAV("580656__therealisa__coindtouch-single-short2.WAV");
        //Creation de la fenetre
        window = SDL_CreateWindow("Game0",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,940,551,SDL_WINDOW_SHOWN);
        if(window == NULL)
        {
                    ...
                    if(event.button.button==SDL_BUTTON_LEFT && event.button.clicks==1)
                    {
                        ...
                        for(i=0; i<12; i++)
                        {
                            if((event.button.x>=T[i].x && event.button.x<=T[i].x+100) && (event.button.y>=T[i].y && event.button.y<=T[i].y+100))
                            {
                                ...
                                if(T[i].ligne == player[2].id && Tab[i]!=0)
                                {
